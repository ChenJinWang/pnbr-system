  ...
  <div class="nofind">
    ...
    >
      <el-form :model="form" ref="formdoalog" label-width="80px">
        ...
        <el-form-item label="反馈状态" prop="auditStatus">
           <span style="margin-left: 10px" v-text="form.auditStatus==0?'待处理':(form.auditStatus==1?'已通过':(form.auditStatus==2?'不通过':(form.auditStatus==3?'已售出':(form.auditStatus==4?'捐赠公益':'退回供应商'))))"></span>
        </el-form-item>
...
export default {
  ...
  methods: {
    addHandle(auditStatus) {
             this.$axios.put(`api/feedback/agency/${this.form.id}/${auditStatus}`, null,{headers: {"token": localStorage.getItem("eleToken")}}).then(res => {
                this.$message({
                  type: "success",
                  message: "数据修改成功"
                });
    ...
    saleableHandle(saleable) {
             this.$axios.put(`api/feedback/agency/saleable/${this.form.id}/${saleable}`, null,{headers: {"token": localStorage.getItem("eleToken")}}).then(res => {
                this.$message({
                  type: "success",
                  message: "数据修改成功"
                });
