  ...
  <div class="nofind">
    ...
    >
      <el-form :model="form" ref="formdoalog" label-width="80px">
        ...
        <el-form-item label="湿度" prop="content">
...
export default {
  ...
  methods: {
    addHandle(auditStatus) {
             this.$axios.put(`api/community/auditStatus/${this.form.id}/${auditStatus}`, null,{headers: {"token": localStorage.getItem("eleToken")}}).then(res => {
                this.$message({
                  type: "success",
                  message: "数据修改成功"
                });
    ...
    saleableHandle(saleable) {
          this.$axios.put(`api/community/agency/saleable/${this.form.id}/${saleable}`, null,{headers: {"token": localStorage.getItem("eleToken")}}).then(res => {
            this.$message({
              type: "success",
              message: "数据修改成功"
            });
