...
export default {
  ...
  methods: {
    addHandle(auditStatus) {
             this.$axios.put(`api/pet/auditStatus/${this.form.id}/${auditStatus}`, null,{headers: {"token": localStorage.getItem("eleToken")}}).then(res => {
                this.$message({
                  type: "success",
                  message: "数据修改成功"
                });
    ...
    saleableHandle(saleable) {
          this.$axios.put(`api/pet/agency/saleable/${this.form.id}/${saleable}`, null,{headers: {"token": localStorage.getItem("eleToken")}}).then(res => {
            this.$message({
              type: "success",
              message: "数据修改成功"
            });
