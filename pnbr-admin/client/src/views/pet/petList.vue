<template>
  <div class="staff">
    <div class="staff-top">
      ...
      <PetDialong :dialong="dialong" :form="form" :parameter="parameter" @petData="loadCurrentPagepetList"></PetDialong>
      <div class="page">
        <el-pagination
...
<script>
// @ is an alias to /src
import PetDialong from "../../components/PetDialong";
export default {
  name: "petList",
  data() {
    return {
  ...
  methods: {
    petList(formData) {
      this.$axios
        .post("/api/pet/backstage/list",formData,{headers: {"token": localStorage.getItem("eleToken")}})
        .then(res => {
    ...
    exportToExcel() {
      ...
      require.ensure([], () => {
        ...
        const tHeader = ['id', '反馈人id', '分类', '标题', '图片', '价格', '更新时间', '创建时间'];
        const filterVal = ['id', 'createId','categoryName', 'title','images', 'price','updateTime', 'createTime'
        ];
