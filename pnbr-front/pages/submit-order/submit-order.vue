  ...
  <view class="submit-order">
	...
	<block v-for="(orderItem, orderItemIndex) in orderItems" :key="orderItemIndex">
		<view class="prod-item">
		  <view class="section">
		    <image style="width: 130rpx;height: 130rpx;" :src="orderItem.user.avatarImage?serverUrl+orderItem.user.avatarImage:'../../static/images/icon/head.png'"></image>
  ...
  methods: {
    ...
    loadOrderData: function () {
	  ...
	  if(ths.orderEntry === "1") {
		  ...
		  var ShoppingCartList = uni.getStorageSync("ShoppingCartIds");
		  console.log(ShoppingCartList);
		  // var orderItemMaps = {};
		  // for(var i=0;i<ShoppingCartList.length;i++) {
			 //  for(var j=i+1;j<ShoppingCartList.length;j++) {
				//   if(ShoppingCartList[i].sellerId===ShoppingCartList[j].sellerId) {
				// 	  var key  =  ShoppingCartList[i].sellerId;
				// 	  var items = orderItemMaps[key];
				// 	  items.push(ShoppingCartList[i]);
				// 	  items.push(ShoppingCartList[j]);
				// 	  orderItemMaps[key] = items;
				// 	  break;
				//   }else {
				// 	  var key1  =  ShoppingCartList[i].sellerId;
				// 	  var key2  =  ShoppingCartList[j].sellerId;
				// 	  var items1 = orderItemMaps[key1];
				// 	  var items2 = orderItemMaps[key2];
				// 	  if(items1 instanceof Array) {
				// 		  items1.push(ShoppingCartList[i]);
				// 	  }else {
				// 		  items1=[];
				// 		  items1.push(ShoppingCartList[i]);
				// 	  }
				// 	  if(items2 instanceof Array) {
				// 	  	  items2.push(ShoppingCartList[j]);
				// 	  }else {
				// 	  	  items2=[];
				// 		  items2.push(ShoppingCartList[j]);
				// 	  }
		  ...
		  ShoppingCartList.forEach(ShoppingCart => {
			  console.log(ShoppingCart.backgroundAgentId)
			    var totalPay = 0;
			    var actualPay = 0;
			    var subtotalCount = 0;
			    ShoppingCart.ShoppingCartProds.forEach(function(item,index) {
			    				  totalCount += item.num;
			    				  totalPay += item.num*item.price;
			  				      subtotalCount +=item.num;
			    });
			    actualPay += totalPay+ths.transfee;
			    total += actualPay;
			    var orderItem = ShoppingCart.ShoppingCartProds;
			    var item = {
			    	prods: orderItem,
			    	totalPay: totalPay,
			    	actualPay: actualPay,
					...
					remark: '',
					backgroundAgentId: ShoppingCart.backgroundAgentId
			    };
			  orderItems.push(item);
		  });
		   ths.setData({
