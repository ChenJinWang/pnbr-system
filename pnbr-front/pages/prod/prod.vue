...
<view class="container">
  ...
  <view class="sku" @tap="showSku">
    <view class="sku-tit">含水量</view>
    <view class="sku-con">{{content}}</view>
      ...
      <view class="cmt-items">
        <view v-for="(item, index) in prodCommPage.records" :key="index" class="cmt-item">
		  ...
		  <view v-for="(subItem, subIndex) in item.subEvaluationRowVos.list" :key="subIndex" class="cmt-reply">
			...
			<view class="cmt-cnt" @tap="openLeaveMessage" :data-="subItem.commentator" :data-index="index" style="padding-left: 50rpx;" :data-id="item.id" :data-commentator="subItem.commentator" :data-toUserType="subItem.creatorType" :data-toUserId="subItem.creatorId">回复{{item.commentator}}: {{subItem.content}}</view>
		  </view>
...
export default {
  ...
  methods: {
	...
	openLeaveMessage: function (e) {
		...
		let toUserName = e.currentTarget.dataset.commentator;
		this.setData({
