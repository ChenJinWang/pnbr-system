...
<view class="container">
  ...
  <view class="order-tit">
	...
	<text @tap="onConsignTap" data-tap="5" :class="tap==5?'on':''">货架</text>
  </view>
  <!-- end 头部菜单 -->
  <view class="main">
      ...
      <view class="prod-item">
        <view class="order-num">
          ...
          <view class="order-state">
			...
			<text v-if="item.type==1" :class="'order-sts  ' + (item.auditStatus==1?'gray':'') + '  ' + ((item.auditStatus==0||item.auditStatus==2||item.auditStatus==3)?'red':'')">{{item.auditStatus==0?'待通过':(item.auditStatus==1?'已通过':(item.auditStatus==2?'不通过':(item.auditStatus==3?'已售出':(item.auditStatus==4?'捐赠':'退回供应商'))))}}</text>
        ...
        <view class="prod-foot">
          <view class="btn">
           <text v-if="item.auditStatus==1&&item.saleable" class="button" @tap="updateSaleable" :data-id="item.id" data-saleable="false" hover-class="none">下架</text>
            ...
            <text v-if="item.type==1&&item.auditStatus==1" class="button warn" @tap="donate" :data-id="item.id" hover-class="none">捐赠公益</text>
			<text v-if="item.type==1&&item.auditStatus==1" class="button warn" @tap="Mailhome" :data-id="item.id" hover-class="none">退回供应商</text>
          </view>
        </view>
      </view>
...
export default {
  ...
  methods: {
	...
	updateAuditStatus: function (id,auditStatus) {
		var ths = this;
		var params = {
		  url: `/pet/auditStatus/${id}/${auditStatus}`,
		  method: "PUT",
		  needToken: true,
		  callBack: function (res) {
		    ths.loadOrderData(ths.sts,ths.type, 1);
