    ...
    <block v-for="(item, index) in orderList" :key="index">
      <view class="prod-item">
        ...
        <view class="prod-foot">
          <view class="btn">
            <text v-if="item.status==1 || item.status==2 & item.status==2 || item.status==1" class="button" @tap="cancelOrder" :data-ordernum="item.orderId" hover-class="none">取消订单</text>
            <text v-if="item.status==1" class="button warn" @tap="normalPay" :data-ordernum="item.orderId" hover-class="none">付款</text>
            <text v-if="item.status==4" class="button" @tap="toDeliveryPage" :data-ordernum="item.orderId" hover-class="none">查看物流</text>
