...
<!--    <text @tap="onStsTap" data-sts='-1' data-tap="5"  :class="tap==1?'on':''">全部</text>
	...
	<text @tap="onConsignTap" data-sts='-1' data-tap="5" :class="tap==5?'on':''">查看</text>
  </view>
  <!-- end 头部菜单 -->
  <view class="main">
...
export default {
  ...
  methods: {
	...
	updateSaleable: function (e) {
		...
		var ths = this;
		var params = {
		  url: `/feedback/saleable/${id}/${saleable}`,
		  method: "PUT",
		  needToken: true,
		  callBack: function (res) {
	...
	updateAuditStatus: function (id,auditStatus) {
		var ths = this;
		var params = {
		  url: `/feedback/auditStatus/${id}/${auditStatus}`,
		  method: "PUT",
		  needToken: true,
		  callBack: function (res) {
		    ths.loadOrderData(ths.sts,ths.type, 1);
