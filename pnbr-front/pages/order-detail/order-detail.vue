...
<view class="container">
  ...
  <view class="order-detail">
    ...
    <view class="order-detail-footer">
      <text class="dele-order" @tap="delOrderList" v-if="status==4&status==5">删除订单</text>
      <view class="footer-box">
		 
		 <text class="dele-order" @tap="onCancelOrder" v-if="status==1 || status==2 & status==2 || status==1">取消订单</text>
		 <text v-if="status==1" class="buy-again" @tap="onPayAgain" hover-class="none">付款</text>
		 <text v-if="!status==4" class="buy-again" @tap="toDeliveryPage" hover-class="none">查看物流</text>
		 <text v-if="!status==3" class="buy-again" @tap="onConfirmReceive" hover-class="none">确认收货</text>
         <text class="apply-service" @tap="toCustomerServiceChat" :data-touserid='sellerId' :data-username='sellerName'>联系商家</text>
		 <text class="apply-service" @tap="toCustomerServiceChats" :data-touserid='sellerId' :data-username='sellerName'>联系客服</text>
